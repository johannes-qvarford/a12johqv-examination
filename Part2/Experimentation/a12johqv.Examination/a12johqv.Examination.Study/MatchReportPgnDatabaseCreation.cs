namespace a12johqv.Examination.Study
{
    ...
    /// Utility functions for creating pgn databases from match reports.
    public static class MatchReportPgnDatabaseCreation
    {
        public static Pgn.Database CreateDatabase(MatchReport matchReport)
        {
            var database = new Pgn.Database();
            database.Games.Add(CreateGame(matchReport));
            return database;
        }

        private static Pgn.Game CreateGame(MatchReport matchReport)
        {
            return new Pgn.Game
                           {
                               ...
                               Result = ConvertChessToPgnResult(matchReport.Result),
                               WhitePlayer = matchReport.GetPlayerReport(Color.White).Player.Name,
                               BlackPlayer = matchReport.GetPlayerReport(Color.Black).Player.Name,
                               MoveText = CreateMoveText(matchReport.Moves)
                           };
        }
