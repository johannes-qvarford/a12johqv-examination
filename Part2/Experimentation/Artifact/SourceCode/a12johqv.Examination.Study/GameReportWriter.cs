namespace a12johqv.Examination.Study
{
    ...
    public class GameReportWriter
    {
        ...
        public GameReportWriter(TextWriter writer)
        {
            this.writer = writer;
        }

        public void WriteGameReports(IEnumerable<GameReport> gameReports)
        {
            var root = new XElement("gameReports", gameReports.Select(GameReportToXElement));
            root.WriteTo(new XmlTextWriter(this.writer){ Formatting = Formatting.Indented });
        }

        private static XElement GameReportToXElement(GameReport gameReport)
        {
            return new XElement(
                "gameReport",
                new[] { Color.White, Color.Black }
                    .Select(color => new { Color = color, PlayerReport = gameReport.GetPlayerReport(color) })
                    .Select(pair =>
                        new XElement(
                            ...
                            new XElement("decisionTimeOfFirst40MovesInMinutes", pair.PlayerReport.DecisionTimeOfFirst40Moves.TotalMinutes))),
                new XElement("result", gameReport.Result),
