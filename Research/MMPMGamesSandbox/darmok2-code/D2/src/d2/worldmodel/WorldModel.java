...
public class WorldModel extends D2Module {
    ...
    public GameState simulate(int cycle, GameState gs, int cycles, int granularity, String player,
        Sensor winGamegoal) {
        Set<String> playerNames = gs.getAllPlayers();

        for (int i = cycle; i < cycle + cycles; i += granularity) {

            // If the game is already won or lost, return, otherwise simulate!
    ...
    public GameState simulate(int cycle, GameState a_gs, Plan plan, String playerID, int cycles,
        int granularity, Sensor winGamegoal) {
        GameState working_gs = (GameState) a_gs.clone();
        Planner pe = new NothingPlanner(plan, null, playerID, new RealTimePlanExecution(playerID,
            new ParameterAdaptation(), new NothingPlanAdaptation(), null), null);
        List<Action> newActions = new LinkedList<Action>();
        Set<String> opponentNames = null;
        boolean planFinished = false;
        // TODO: make this a parameter of the simulator:
        int planFinishedMargin = 64; // number of cycles to simulate after
        // the plan is over
     ...
     * @return
     */
    public List<GameState> logSimulation(int cycle, GameState a_gs, Plan plan, String playerID,
        int cycles, int granularity, Sensor winGamegoal) {
        GameState working_gs = (GameState) a_gs.clone();
        Planner pe = new NothingPlanner(plan, null, playerID, new RealTimePlanExecution(playerID,
            new ParameterAdaptation(), new NothingPlanAdaptation(), null), null);
        List<Action> newActions = new LinkedList<Action>();
