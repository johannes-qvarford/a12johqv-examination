 ...
 * @author santi
 */
public class OverlappedChunksPlanLearning extends PlanLearning {
    public static int DEBUG = 1;
    protected Sensor m_winGameGoal;
    int m_chunkSize = 3;

    public OverlappedChunksPlanLearning(IDomain domain) {
        super(domain);
        m_winGameGoal = domain.getWinGoal();
    }

    
    public OverlappedChunksPlanLearning(IDomain domain, double chunkSize) {
        super(domain);
        m_winGameGoal = domain.getWinGoal();
    ...
    public List<PlanBaseEntry> learnFromTrace(Trace t, String playerName, WorldModel wm) {
        ...
        for(int idx = 0;idx<entry_list.size();idx++) {
            ...
            for(int idx2 = 0, currentSize = 0;idx+idx2<entry_list.size() && currentSize<m_chunkSize;) {
                Entry e = entry_list.get(idx+idx2);
