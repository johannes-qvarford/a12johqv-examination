...
//			input = new java.io.FileInputStream(new java.io.File(u));
			input = getClass().getResourceAsStream("/gatech/mmpm/tools/nativedomain/NativeDomain.xml");
		} catch (Exception ex) {
                        ex.printStackTrace();
			System.err.println("Native domain specification could not be found (in " + u +")");
			return null;
		}
