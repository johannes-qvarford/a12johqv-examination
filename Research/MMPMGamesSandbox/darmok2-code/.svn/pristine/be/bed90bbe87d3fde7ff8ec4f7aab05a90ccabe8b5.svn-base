		...
		if (plan instanceof PetriNetPlan) {
			...
			for(PetriNetElement e:pn.getPetriNetElements()) {
				...
				if (e instanceof Transition) {
					...
					if (e instanceof PreTransition ||
							e instanceof SuccessTransition ||
							e instanceof FailureTransition) {
						if (recurse>0) {
							if (m_all_elements.get(e.getPlan())==null) createPlanElements(e.getPlan(),frame,recurse-1);
							f.m_links.add(new WeakLocationLink(e,e.getPlan()));	
						}
...
//						distance = l1.distance(l2); 
			...
			for(LocationLink l:links) { 
				...
				if (l1!=null && l2!=null && l1!=l2 && !(l instanceof WeakLocationLink)) {
					...
					if (l1.m_element instanceof PlanState &&
						l2.m_element instanceof PreTransition &&
							((PlanState)l1.m_element).getPlan()==((PreTransition)l2.m_element).getPlan()) {
						desired_offset_x = 0;
						desired_offset_y = -desired_distance;
						desired_offset = true;
					}
					if (l1.m_element instanceof PreTransition &&
						l2.m_element instanceof PlanState &&
							((PlanState)l2.m_element).getPlan()==((PreTransition)l1.m_element).getPlan()) {
						desired_offset_x = 0;
						desired_offset_y = desired_distance;
						desired_offset = true;
					}
					if (l1.m_element instanceof PlanState &&
						l2.m_element instanceof SuccessTransition &&
							((PlanState)l1.m_element).getPlan()==((SuccessTransition)l2.m_element).getPlan()) {
						desired_offset_x = 0;
						desired_offset_y = desired_distance;
						desired_offset = true;
					}
					if (l1.m_element instanceof SuccessTransition &&
						l2.m_element instanceof PlanState &&
							((PlanState)l2.m_element).getPlan()==((SuccessTransition)l1.m_element).getPlan()) {
						desired_offset_x = 0;
						desired_offset_y = -desired_distance;
						desired_offset = true;
					}
					if (l1.m_element instanceof PlanState &&
						l2.m_element instanceof FailureTransition &&
							((PlanState)l1.m_element).getPlan()==((FailureTransition)l2.m_element).getPlan()) {
						desired_offset_x = -desired_distance;
						desired_offset_y = 0;
						desired_offset = true;
					}
					if (l1.m_element instanceof FailureTransition &&
						l2.m_element instanceof PlanState &&
							((PlanState)l2.m_element).getPlan()==((FailureTransition)l1.m_element).getPlan()) {
						desired_offset_x = desired_distance;
						desired_offset_y = 0;
						desired_offset = true;
					}				
...
//							if (offset_x>1000 || offset_y>1000 ||
...
/*
			for(Location l:elements.values()) {
				...
				int gx = (int)((l.m_x+(grid_x/2))/grid_x);
				int gy = (int)((l.m_y+(grid_y/2))/grid_y);

				offset_x = (gx*grid_x)-l.m_x;
				offset_y = (gy*grid_y)-l.m_y;

				pushes.get(l).add(new Push("Grid",offset_x,offset_y,weight_grid));
			}
*/
