...
public class RuleList extends AssocRuleMining {
	...
	protected class RuleNode {
		...
		/** The Laplace accuracy associate with the rule represented by this node. */
	...
	/**
	 * Inserts an (association/classification) rule into the linked list of rules pointed at 
	 * by startRulelist</TT>.  The list is ordered so that rules with highest confidence are 
	 * listed first. If two rules have the same confidence the new rule will be placed after 
	 * the existing rule. Thus, if using an Apriori approach to generating rules, more general 
	 * rules will appear first in the list with more specific rules (i.e. rules with a larger
	 ...
	 * @param laplaceAccuracy the associated Laplace accuracy value for the rule.
	 */

	protected void insertRuleintoRulelist(short[] antecedent, short[] consequent, double laplaceAccuracy) {
		// Create new node
	...
	/**
	 * Determines Laplace expected error estimates (accuracies).  Calculated as follows:
	 * 
	 * Nc   = Number of records in training set that include all the attributes for the given rule 
	 * 		  antecedent + consequent, i.e. the total support for the rule.
	 * Ntot = Number of records in training set that include all the attributes in the rule's antecedent, 
	 * 		  i.e. support for antecedent.
	...
	/**
	 * Selects the best rule in a rule list according to the average expected Laplace accuracy. 
	 * Operates as follows:
	 * 1) Obtain all rules whose antecedent is a subset of the given record.
	 * 2) Select the best K rules for each class (according to their Laplace accuracy).
	 ...
	 * 
	 * @param kValue the maximum number of rules to be considered to classify the given record.
	 * @param classification the possible classifiers.
	 * @param itemset the record to be classified.
	 * @return the class label.
	 */
	protected short classifyRecordBestKaverage(int kValue, short[] classification, short[] itemSet) {	
	...
	/**
	 * Keep only best K rules for each class. Note rules are stored according to Laplace 
	 * accuracy (most accurate rule first), this is the measure to be maximized to identify 
	 * "best" rules.
	 ...
	 * @param decrement the label for the first class in the classification array which is used here 
	 * to relate class labels to indexes.
	 ...
	 * @param increment the label for the first class in the classification array which is used here to 
	 * relate class labels to indexes.
	...
	/**
	 * Places all rules that satisfy the given record in a rule linked list pointed at by 
	 * startRulelist field, in the order that rules are presented.
	...
	private void obtainallRulesForRecord(RuleNode linkRef, short[] itemSet) {
		...
		while (linkRef!=null) {
			...
			if (isSubset(linkRef.antecedent,itemSet)) {
				RuleNode newNode = new RuleNode(linkRef.antecedent, linkRef.consequent,linkRef.laplaceAccuracy);
	...
	/**
	 * Returns the number of generated rules (usually used in conjunction with classification 
	 * rule mining algorithms rather than ARM algorithms).
	...
	private void outputRules(RuleNode ruleList) {
		...
		while (linkRuleNode != null) {
			...
			System.out.println(" " + twoDecPlaces(linkRuleNode.laplaceAccuracy) + "%");
			number++;
			linkRuleNode = linkRuleNode.next;
		}
	}
