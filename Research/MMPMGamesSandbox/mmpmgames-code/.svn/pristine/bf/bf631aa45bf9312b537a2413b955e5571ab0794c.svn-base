...
public class TUpgradeGold extends PhysicalEntity {

	public TUpgradeGold(String owner, String id,int x,int y) {
		super(owner, id);
		coords[0]=x;
		coords[1]=y;
		width = 16;
	...
	public boolean cycle(TMap map, Towers game, List<Action> actions) throws IOException {
		if (m_lastTileUsed==null) {
			...
			m_lastTileUsed = SpriteManager.get("upgradeGold");
	...
	public gatech.mmpm.Entity toD2Entity() {
		towers.mmpm.entities.TUpgradeGold ret;
		
		ret = new towers.mmpm.entities.TUpgradeGold(entityID, owner);
		ret.setx(coords[0]);
		ret.sety(coords[1]);
		return ret;
	}
