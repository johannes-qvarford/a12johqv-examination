...
import s3.ai.builtin.FootmenRush2;
...
public class S3App extends Canvas {
    ...
    private static final String AUTHORS = "Santiago Ontanon Villar (2012)";
    ...
    public void gameLoop(int maxCycles) {
        ...
        if (tracer != null) {
            ...
            prop.setProperty("domain", d2.core.Config.getDomainName());
        ...
        System.out.println("Tracer completed...");                
    ...
    public S3App(String map, int traceInterval, List<PlayerInput> players,
        ...
        System.out.println("S3: Player setup complete.");
        ...
        for (PlayerInput pi : players) {
            ...
            if (pi.m_inputType == PlayerInput.INPUT_AI) {
                ...
                if (pi.AIType.equals("ai-empty")) {
                    ...
                    ai = new FootmenRush2(pi.m_playerID);
                    ai.gameStarts();
                    ...
                    if (pi.ME!=null) {
                        ai = new AI_D2(pi.ME, pi.m_playerID, idomain);
                        ai.gameStarts();
                    } else if (!pi.MEFile.isEmpty()) {
                        ai = createD2IA(pi);
                        ai.gameStarts();
                    } else {
                    ...
                    try {
                        ...
                        ai = (AI)ctr.newInstance(pi.m_playerID);
                        ai.gameStarts();
                    } catch(Exception e) {
                        e.printStackTrace();
    ...
    private void performActions(List<S3Action> actions) {
        for (S3Action action : actions) {
            ...
            if (u != null) {
                u.performAction(action, getGame());
            }
        }
    }
    ...
    private AI createD2IA(PlayerInput pi) throws Exception {
        IMEExecutor meExec = MEExecutorFactory.BuildMEExecutor(pi.MEFile, idomain);
