...
public class S3 {
     ...
     * Default constructor.
     *
     * @param game_doc game definition.
     * @throws Exception
     */
    public S3(Document game_doc) throws Exception {
        ...
        for (Element entity : entities) {
            ...
            if (entity_type.equals("map")) {
                ...
                if (entity_type.equals("WPlayer")) {
                    ...
                    Class<? extends S3Entity> entityClass = (Class<? extends S3Entity>) Class.forName("s3.entities." + entity_type);
        ...
        for(WUnit u:units) {
            if (u instanceof WPeasant) ((WPeasant)u).setAllowed(this);
            if (u instanceof WBuilding) ((WBuilding)u).setAllowed(this);
        }
    ...
    private WUnit getEntityFromElement(Element entity) throws ClassNotFoundException,
        ...
        if (entityName.equals("WFarm")) {
            ...
            entityClass = (Class<? extends WUnit>) Class.forName("s3.entities." + entityName);
     ...
     * @param mapEntityType type of the map entity
     * @param home find the wood closest to this entity
    ...
    /**
     * Returns the all the instances of the given unit for the given player.
     *
     * @param player
     * @param type
     * @param game
     * @return
     */
    public List<WUnit> getAllUnitsByType(WPlayer player, Class<? extends WUnit> unitType) {
        List<WUnit> ret = new LinkedList<WUnit>();
        for (WUnit unit : units) {
            if (unit.getClass().equals(unitType)) {
                if (player == null && unit.owner == null) {
                    ret.add(unit);
                }
                if (player != null && unit.owner.equals(player.owner)) {
                    ret.add(unit);
    ...
    /*
     * Adds a new unit to the game state directly, without adding it to "newUnits" 
     * (not recommended, unless you know what you are doing)
     * 
     * @ param unit
     */
    public void addUnitDirectly(WUnit unit) {
        units.add(unit);
    }
