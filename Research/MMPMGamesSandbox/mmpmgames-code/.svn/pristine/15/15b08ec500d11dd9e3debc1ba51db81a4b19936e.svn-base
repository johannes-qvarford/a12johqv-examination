...
public class WPeasant extends WTroop {
    ...
    public void performAction(S3Action a, S3 game) {
    ...
    public boolean isAllowedToBuild(String type) {
//        for(String s:allowedUnits) System.out.println(s);
        return allowedUnits.contains(type);
    }
     ...
     * @param m_game
     */
    public void setAllowed(S3 m_game) {
    ...
    private void doBuild(int cycle, S3 game, List<S3Action> failedActions) {
        ...
        if (!allowedUnits.contains(type)) {
            System.out.println("Can't build '" + type + "' because of unsatisfied dependencies");
            // cancel the action
            status = null;
            progressTimer = progressTimerMax = 0;   
            return;
...
/*               
                // System.out.println();
                XMLWriter w = new XMLWriter(new PrintWriter(System.out));
                Game2D2Converter.toGameState(game, d2.core.Config.getDomain()).writeToXML(w);
                w.flush();                        
*/               
    ...
    private void chop(int xTarget, int yTarget, S3 game) {
        if (carriedWood == 100) {
//            System.out.println("Peasant " + entityID + " going towards town hall...");
            WPlayer player = getPlayer(game);
...
//                System.out.println("Peasant " + entityID + " in range of target...");
                S3PhysicalEntity wood = game.mapEntityAt(xTarget, yTarget);
                if (wood==null ||
                    !(wood instanceof WOTree)) {
                    WPlayer player = getPlayer(game);
                    // already harvested
...
//                        System.out.println("Peasant " + entityID + " changing location to: " + nextWood.getX() + "," + nextWood.getY());
                        return;
                    }
//                    System.out.println("Peasant " + entityID + " stops harvesting wood...");
                    if (player.getInputType() == PlayerInput.INPUT_MOUSE) {
