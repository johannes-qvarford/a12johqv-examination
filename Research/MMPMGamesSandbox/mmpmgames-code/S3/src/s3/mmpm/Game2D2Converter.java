...
public class Game2D2Converter {
    ...
    public static boolean reserveGoldForBuild = false;
    ...
    public static List<Action> toD2Action(S3Action action, GameState game, String playerID) {
        ...
        if (action.m_action == S3Action.ACTION_ATTACK) {
            ...
            if (target != null) {
                ...
                System.err.println("Game2D2Converter.toD2Action: Null target in Attack action, ignoring!");
            }
    ...
    public static gatech.mmpm.GameState toGameState(s3.base.S3 game, gatech.mmpm.IDomain idomain) {
        ...
        // First pass: detect any units with an issued but unstarted "build action":
        List<WPeasant> peasantsWithBuild = new LinkedList<WPeasant>();
        for (s3.entities.S3Entity e : entities) {
            if (e instanceof WPeasant) {
                WPeasant p = (WPeasant)e;
                S3Action a = p.getStatus();
                if (a!=null && a.m_action == S3Action.ACTION_BUILD &&
                    p.getProgressTimerMax()==0) peasantsWithBuild.add(p);
            }
        }
        ...
        for (s3.entities.S3Entity e : entities) {
            ...
            if ((d2Entity != null)) {
                if (reserveGoldForBuild &&
                    d2Entity instanceof s3.mmpm.entities.WPlayer) {
                    // Assess the amount of gold and wood that has been reserved for build actions, 
                    // but that S3 has not yet subtracted form the player counts:
                    s3.mmpm.entities.WPlayer d2p = (s3.mmpm.entities.WPlayer)d2Entity;
                    
                    int goldToReserve = 0;
                    int woodToReserve = 0;
                    String playerName = e.getOwner();
                    for(WPeasant p:peasantsWithBuild) {
                        if (p.getOwner().equals(playerName)) {
                            S3Action a = p.getStatus();
                            String type = (String)a.m_parameters.get(0);
                            WBuilding building = (WBuilding) p.newEntity(type);
                            goldToReserve += building.getCost_gold();
                            woodToReserve += building.getCost_wood();
                        }
                    }

                    // The values might be smaller than 0, since D2 sometimes messes up and sends two
                    // actions at a time, for which individually there are enough resourcees, but not
                    // if they are executed in parallel.
                    d2p.setGold(Math.max(0,d2p.getGold() - goldToReserve));
                    d2p.setWood(Math.max(0,d2p.getWood() - woodToReserve));
                }
