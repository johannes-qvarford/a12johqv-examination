...
//				System.out.println("  My target out of range");
     ...
     * @param a_game
     */
    private void doStandGround(S3 a_game) {
        for (WUnit unit : a_game.getUnits()) {
     ...
     * @param a_game
     */
    private void doIdleAction(S3 a_game) {
        for (WUnit unit : a_game.getUnits()) {
            if (null == owner || null == unit.owner) {
                continue;
            }
            if (!unit.owner.equals(owner) && inRange(unit)) {
                performAction(new S3Action(entityID, S3Action.ACTION_ATTACK, unit.entityID), a_game);
                return;
            }
        }
    }
     ...
     * @param target
     */
    public void moveTowardsTargetToAttack(S3 m_game, WUnit target) {
...
//		System.out.println("  My target is in " + target.getX() + "," + target.getY() + " but I want to go to " + loc.m_a + "," + loc.m_b + " I am at " + getX() + "," + getY());
     ...
     * @return true if the unit is in range, false otherwise.
     */
    public Pair<Integer, Integer> rangedLoc(WUnit target, S3 game) {
        if (target == null) {
        ...
        for (int xLoc = target.getX() - range; xLoc < target.getX() + target.getWidth() + range; xLoc++) {
            for (int yLoc = target.getY() - range; yLoc < target.getY() + target.getLength() + range; yLoc++) {
     ...
     * @return true if the unit is in range, false otherwise.
     */
    Pair<Integer, Integer> rangedLoc(int goalx, int goaly, S3 game) {
        int bestx = -1;
        int besty = -1;
        double bestdistance = -1;
